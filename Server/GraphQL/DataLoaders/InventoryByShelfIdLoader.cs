...
{
	public class InventoryByShelfIdLoader : GroupedDataLoader<string, ProductInventory>
	{
		private readonly IDbContextFactory<EfContext> _factory;
		public InventoryByShelfIdLoader(IBatchScheduler batchScheduler, DataLoaderOptions options, IDbContextFactory<EfContext> factory) : base(batchScheduler, options)
		{
			_factory = factory;
		}
		...
		protected override async Task<ILookup<string, ProductInventory>> LoadGroupedBatchAsync(IReadOnlyList<string> keys, CancellationToken cancellationToken)
		{
			...
			return inventory.ToLookup(x => x.ShelfId!);
		}
	}
}
